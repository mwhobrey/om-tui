...
| TUI Google Chrome cookie auto-read | Parked for direct DB decrypt: current Chrome encrypts Gaia cookies (v20 / app-bound). Prefer the Chrome MV3 cookie bridge ([05_CHROME_COOKIE_BRIDGE.md](./05_CHROME_COOKIE_BRIDGE.md)) when Chrome is open. Paste a `messages.google.com` curl remains the fallback. If this PC is already paired, paste rewrites cookies and reconnects (no phone tap). The TUI auto-opens that paste overlay when `auth_expired` / `needs_repair` latches and the bridge is offline; Esc dismisses until the session is healthy again. Gaia + emoji only when unpaired or the cookie reconnect fails |
